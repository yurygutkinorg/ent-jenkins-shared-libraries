              ...
              withEnv(["RELEASE_NAME=${RELEASE_NAME}"]) {
                withMaven(mavenSettingsFilePath: 'settings.xml') {
                  script {
                    ...
                    sh(script: "mvn -B clean -Dtoken=$token")
                  }
                }
              }
            }
            ...
            string(credentialsId: "${env.ANYPOINT_KEY_SECRET_NAME}", variable: 'MULESOFT_KEY'),
              ...
              withEnv(["RELEASE_NAME=${RELEASE_NAME}"]) {
                withMaven(mavenSettingsFilePath: 'settings.xml') {
                  script {
                    def token = getConnectedAppToken()
                    sh(script: "mvn -B test -DsecureKey=$MULESOFT_KEY -Dtoken=$token")
                  }
                }
              }
            }
...
String getConnectedAppToken() {
    ...
    def json_response = sh(script: "curl   -Ls -o -X POST -d -H 'grant_type=client_credentials' -u $client_id:$client_secret ${access_token_url}", returnStdout:true)
